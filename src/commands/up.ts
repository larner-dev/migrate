...
export const upCommand = async (
  ...
  let db = null;
  const migrationDir = join(cwd(), dir);
  ...
  const dirents = await readdir(migrationDir, { withFileTypes: true });
  let filter: RegExp | null = null;
  if (options.filter) {
    filter = new RegExp(options.filter as string);
  }
  ...
  try {
    ...
    const lastMigration = await db.query<{ version: string }>(
      'SELECT version FROM migrations ORDER BY "ended_at" DESC LIMIT 1',
    ...
    migrationsToRun = await Promise.all(
      (lastMigration.length
        ...
        version: m,
        sql: (await readFile(join(migrationDir, m))).toString(),
      }))
    );
